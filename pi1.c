...
/* This code is in an unfinished state and as such should not be used
   ...
   hopefully supporting double precision when that is available in hardware.
...
#define DESIRED_MAT_SIZE (16*768)
...
//a simple test case...
void diaginitialize(float(*x)[MAT_SIZE])
{
    int i, j;
...
void initialize(float(*x)[MAT_SIZE])
{
    float tmp[MAT_SIZE];
    int i, j, a, b;
...
void diagonalinitialize(float(*d)[MAT_SIZE], float(*x)[MAT_SIZE])
{
...
void cpucholdc(float(*x)[MAT_SIZE], float(*d)[MAT_SIZE])
{
...
__global__ void d_choldc_topleft(float(*m)[MAT_SIZE],
    int boffset)
{
    int tx = threadIdx.x;
    int ty = threadIdx.y;
...
__global__ void d_choldc_strip(float(*m)[MAT_SIZE],
    int blockoffset)
{
    ...
    __shared__ float topleftt[BLOCK_SIZE][BLOCK_SIZE + 1];
...
__global__ void d_choldc_diagupdate(float(*m)[MAT_SIZE],
    int blockoffset)
{
    int boffx = blockIdx.x + blockoffset + 1;
    ...
    __shared__ float topt[BLOCK_SIZE][BLOCK_SIZE + 1];
...
//rest of the matrix...
__global__ void d_choldc_hiupdate(float(*m)[MAT_SIZE],
    int blockoffset)
{
    ...
    __shared__ float leftt[BLOCK_SIZE][BLOCK_SIZE + 1];
    __shared__ float rightt[BLOCK_SIZE][BLOCK_SIZE + 1];
...
void cpumatdisp(float(*mat)[MAT_SIZE], float(*diag)[MAT_SIZE])
{
    int i, j;
    printf("CPU output: \n");
...
void matdisp(float(*matptr)[MAT_SIZE])
{
    float mat[MAT_SIZE][MAT_SIZE];

    unsigned int mat_size = MAT_SIZE * MAT_SIZE * sizeof(float);
...
void choldc(float(*mat)[MAT_SIZE])
{
    volatile clock_t gputime;

    float(*d_mat)[MAT_SIZE];
    unsigned int mat_size = MAT_SIZE * MAT_SIZE * sizeof(float);
